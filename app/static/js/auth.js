...
async function loginUser() {
    ...
    if (!email || !password) { showError("Veuillez remplir tous les champs.", "login"); return; }
    ...
    try {
        ...
        else window.location.href = result.redirect || "/home";
    } catch (err) { showError("Erreur serveur.", "login"); console.error(err); }
}
...
async function registerUser() {
    ...
    if (!fullname || !email || !password || !confirm) { showError("Veuillez remplir tous les champs.", "register"); return; }
    if (password !== confirm) { showError("Les mots de passe ne correspondent pas.", "register"); return; }

    try {
        const result = await rpcCall("register", { fullname, email, password, confirm_password: confirm, role });
        if (!result.success) showError(result.message || "Erreur inscription", "register");
        else window.location.href = "/login";
    } catch (err) { showError("Erreur serveur.", "register"); console.error(err); }
}
